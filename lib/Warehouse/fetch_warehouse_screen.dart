...
class FetchWarehousesScreen extends StatelessWidget {
  final box = Hive.box('stores');

  @override
  Widget build(BuildContext context) {
    final ref = FirebaseFirestore.instance.collection('stores').doc(box.get('code'));
    final warehouses = FirebaseFirestore.instance.collection(
      'warehouses',
    ).where('store_ref', isEqualTo: ref);

    return Scaffold(
      appBar: AppBar(title: Text("List Warehouse")),
      body: StreamBuilder<QuerySnapshot>(
        stream: warehouses.snapshots(),
                          ...
                          children: [
                            ...
                            IconButton(
                              ...
                              onPressed: () { 
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(builder: (context) => WarehouseEditPage(
                              ...
                              onPressed: () async {
                                bool confirm = await showDialog(
                                  ...
                                  builder: (context) => AlertDialog(
                                    ...
                                    content: Text('Apakah anda yakin ingin menghapus warehouse ini?'),
                                    actions: [
                                      TextButton(
      ...
      floatingActionButton: FloatingActionButton(
        onPressed: () {
          Navigator.push(
            context,
            MaterialPageRoute(builder: (context) => WarehouseFormPage())
          );
        },
