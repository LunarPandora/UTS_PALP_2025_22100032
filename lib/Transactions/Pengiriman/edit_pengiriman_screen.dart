...
class PengirimanEditPage extends StatefulWidget {
  final DocumentReference pengirimanRef;
  final Map<String, dynamic> pengirimanData;

  const PengirimanEditPage({
    super.key,
    required this.pengirimanRef,
    required this.pengirimanData,
  });

  @override
  State<PengirimanEditPage> createState() => _PengirimanEditPageState();
}

class _PengirimanEditPageState extends State<PengirimanEditPage> {
  ...
  void initState() {
    super.initState();
    _formNumberController.text = widget.pengirimanData['no_form'] ?? '';
    _supplierRef = widget.pengirimanData['supplier_ref'];
    _warehouseRef = widget.pengirimanData['warehouse_ref'];
    _loadData();
  }
  ...
  Future<void> _loadData() async {
    ...
    final detailSnapshot = await widget.pengirimanRef.collection('details').get();
  ...
  Future<void> _submitUpdate() async {
    ...
    final oldWarehouseRef = widget.pengirimanData['warehouse_ref'];
    final detailsCollection = widget.pengirimanRef.collection('details');
    final existingDetailsSnapshot = await detailsCollection.get();

    // Rollback old stock
    for (var oldDetail in existingDetailsSnapshot.docs) {
      final productRef = oldDetail['product_ref'] as DocumentReference;
      final qty = oldDetail['qty'] as int;

      final oldStockQuery = await FirebaseFirestore.instance
          .collection('stock')
          .where('store_ref', isEqualTo: storeRef)
          .where('warehouse_ref', isEqualTo: oldWarehouseRef)
          .where('product_ref', isEqualTo: productRef)
          .get();

      if (oldStockQuery.docs.isNotEmpty) {
        final stockRef = oldStockQuery.docs.first.reference;
        await stockRef.update({'stock': FieldValue.increment(qty)});
      }
    }
    ...
    await widget.pengirimanRef.update({
    ...
    for (var detail in _details) {
      ...
      final stockQuery = await FirebaseFirestore.instance
          .collection('stock')
          .where('store_ref', isEqualTo: storeRef)
          .where('warehouse_ref', isEqualTo: _warehouseRef)
          .where('product_ref', isEqualTo: productRef)
          .get();
  ...
  Future<void> _promptDeletePengiriman() async {
    final confirm = await showDialog<bool>(
      context: context,
      builder: (_) => AlertDialog(
        title: Text('Konfirmasi'),
        content: Text('Yakin ingin menghapus pengiriman ini? Semua detail akan ikut terhapus.'),
        actions: [
    ...
    if (confirm == true) {
      final storeRef = FirebaseFirestore.instance.collection('stores').doc(box.get('code'));
      final detailDocs = await widget.pengirimanRef.collection('details').get();
      ...
      await widget.pengirimanRef.delete();
      await FirebaseFirestore.instance.collection('outgoingReceipts').doc(widget.pengirimanRef.id).delete();
              ...
              child: Form(
                ...
                child: ListView(
                  children: [
                    ...
                    SizedBox(height: 10),
                    ElevatedButton(
                      onPressed: _promptDeletePengiriman,
                      style: ElevatedButton.styleFrom(
                        padding: EdgeInsets.all(20),
                        shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(10)),
                        backgroundColor: Colors.red.shade700,
...
class _ProductDetail {
  ...
  factory _ProductDetail.fromFirestore(
      Map<String, dynamic> data, List<DocumentSnapshot> products, DocumentReference ref) {
